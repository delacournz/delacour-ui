...
/**
 * The seventeen shadcn accents, minus the grey each one smuggles in.
 *
 * An accent repaints `primary`, the five-step chart ramp and the sidebar's
 * primary — nine tokens — and nothing else, because it is meant to sit on top of
 * whichever base colour is already active.
 *
 * shadcn also ships `secondary` and `secondary-foreground` in every one of the
 * seventeen, and it is the same zinc grey every time —
 * `oklch(0.967 0.001 286.375)` light, `oklch(0.274 0.006 286.033)` dark —
 * regardless of the base colour underneath. Carrying that across would repaint
 * `Button variant="secondary"` and every `ListGroup` surface zinc the moment an
 * accent was picked, so a stone app would go quietly grey in exactly the places
 * a user notices last. Leaving the pair out lets the base colour's own secondary
 * survive, which is the entire reason the two axes are separate.
 *
 * One further stray goes with them: shadcn's `rose` alone declares a tinted
 * `sidebar` in its dark variant and in no other accent, which would give one of
 * the seventeen a sidebar the other sixteen do not have. All nine keys, both
 * variants, all seventeen — or the axis is not an axis.
 *
 * The values are copied out of shadcn's `THEMES` byte for byte, for the reason
 * given on `BASE_COLORS` in `./base-colors`.
 */
