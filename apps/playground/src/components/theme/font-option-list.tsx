import { SYSTEM_FONT } from "@delacour/design-system/config";
...
export type FontOptionListProps = {
	...
	withInherit?: boolean;
	/** Prepend the "System — the platform's own font" row. Body only. */
	withSystem?: boolean;
};
 ...
 * have to be memoised.
 *
 * `withInherit` and `withSystem` are booleans rather than a mode: each adds
 * one row and changes nothing else, and an enum for that would be a prop
 * pretending to be a decision. They are never both set — a heading says "no
 * face" with `inherit`, a body says it with `system` — but nothing here has to
 * know that.
 *
 * The System row's preview names no family, so `Text` draws it in the platform
 * font — which is the only honest specimen for a value that means exactly that.
 *
 ...
 */
export function FontOptionList({ selected, onSelect, withInherit, withSystem }: FontOptionListProps): ReactElement {
	return (
		<View className="gap-3">
			{withSystem ? (
				<ListGroup isDivided={false} variant="transparent">
					<ListGroup.Item
						className={selected === SYSTEM_FONT ? AXIS_SELECTED_ROW_CLASS : undefined}
						haptic="selection"
						onPress={() => onSelect(SYSTEM_FONT)}
						testID="theme-option-font-system"
					>
						<ListGroup.ItemContent>
							<ListGroup.ItemTitle>System</ListGroup.ItemTitle>
							<ListGroup.ItemDescription>The platform's own font</ListGroup.ItemDescription>
						</ListGroup.ItemContent>
