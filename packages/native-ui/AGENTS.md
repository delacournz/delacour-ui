...
`Accordion.Indicator` and `Accordion.Content`. It takes **ListGroup**'s axes and
its divider insertion outright, so read that section first — what follows is only
where a disclosure differs from a list of rows.
...
- **The panels are the order of record, and they alone must be direct children.**
  The root walks its own children for `Tabs.Content`, exactly as `ListGroup`
  finds a hand-placed `Separator`, and their source order is what `position`
  indexes into. Triggers are **not** walked: each finds its place by looking its
  `value` up in that list, so it can sit behind a wrapper or come out of a
  `.map()`. Walking triggers instead breaks the first time anyone writes
  `<FilterTabs />` — the walk sees a component it does not recognise and silently
  drops tabs. A bar with no panels at all is a real shape (a filter row driving a
  list elsewhere) and falls back to trigger registration, which is one commit
  late and therefore never preferred when panels exist.
...
- **The indicator animates `width` and `translateX`, never `scaleX`.** A scale is
  ...
  sibling, so the row's layout stays settled. `Checkbox` already animates a width
  every toggle.
- **The pan claims sideways and gives up vertical**, `activeOffsetX` under
  ...
  whole mechanism, and it is what lets a pager live inside `Screen.ScrollArea`.
  `blocksExternalGesture` is **not** the alternative — `Screen.ScrollArea`
  renders React Native's own `ScrollView`, which has no handler tag to resolve,
  so the call is dropped without an error. `Slider.Track` found that first, and
  reaches the opposite conclusion for the opposite reason: a slider must claim
  even a stationary tap, so it takes `minDistance(0)` and no axis offsets at all.
- **The settle lives in `onFinalize`, and the spring is cancelled in `onStart`.**
  ...
  CANCELLED alike, from any state — the rule `Slider.Track` already states — so
  it owns the settle, guarded by a flag so a touch that never activated cannot
  retarget a spring it never disturbed. Reproduce the old bug by swiping and then
  scrolling the page before the spring has finished.
...
- **The axis ladder is `own ?? root ?? default`, `Checkbox`'s and not
  ...
  set. There is no `Field` rung and no `isInvalid`: tabs are navigation, nothing
  here has a value that can be wrong, and a `Field` around a `Tabs` would grey
  out a navigation bar from a form's state.
- **The disabled fade lands on the label, never on the trigger.** The `trigger`
  slot is worn by `Pressable`'s own `Animated.View`, whose `useAnimatedStyle`
  writes `opacity` every frame — `Radio`'s lesson, and it bites twice here
  because `feedback` defaults to `fade`.
...
- **No `--spacing-tabs-*`, and that is a decision.** A tab lines up against no
  ...
  clipping the label. `Badge`'s "a size is padding, never a height", and
  `Checkbox`'s argument against minting a private scale.
...
- **Reduce motion takes the default `System` policy**, `Checkbox`'s call rather
  than `Spinner`'s. The state change is the point and the travel is decoration.
  One wrinkle worth writing down: **the drag itself is unaffected and must be** —
  `onUpdate` writes `position.value` directly, with no `withSpring` to shorten. A
  direct manipulation is the user's own hand, not motion played at them; only the
  release settle degrades.
- **There is no `Tabs.ListBackground` and no `background` prop.**
  `Screen.Navbar.Background` exists because that part overlays content and its
  backing has to travel with the keyboard translation. A tab track is a static
  box in the flow, `bg-muted` on the `list` slot is the whole feature, and
  `variant` already says whether there is one.
- **There is no public pager, no `orientation`, and no router prop.** The root
  wraps its own panels the way `ListGroup` inserts its own dividers; a vertical
  tab bar is a sidebar, a different component whose measurements are not in x;
  and this library takes no navigation dependency, so `onValueChange` is where an
  app calls its router — `Screen.Navbar.BackButton`'s trade.
- **No `hitSlop`, and no wash layers.** A trigger is already a wide row, and slop
  ...
  ambiguous — `Checkbox`'s rule. The capsule sliding under the label is the
  feedback; `feedback="fade"` on top of it is the press.
...
- **Two parts are not drawn where they are written, and that is the component.**
  ...
  its own children — the rule `withIndicator` follows in `radio.tsx` — and the
...
- **`BottomSheet.Footer` is inline by default, and `sticky` pins it.** Same word,
  same default as `Screen.Footer`, so the two cannot be read as opposites. The
...
- **The footer publishes its measured height and the body reserves it with an
  ...
  nothing on the JS thread; `Screen` reserves its own chrome exactly this way.
...
- **Do not put a `KeyboardStickyView` inside a sheet.** This is the one keyboard
  rule that differs from `Screen`. gorhom's `animatedFooterPosition` already
...
- **Two `withUniwind` wrappers, and only two.** `BottomSheetBackdrop` and
  ...
  the move `Screen.Footer` already makes with `KeyboardStickyView`. Reach for a
...
- **`accessible` defaults to `false`, and it is the least obvious line in the
  ...
  `Field`'s rule one component along: the row is `accessible={false}` so the
