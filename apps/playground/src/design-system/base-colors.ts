/**
 * A raw shadcn token name, without the leading `--`.
 *
 * The raw name is the one that survives to runtime: `@theme inline` substitutes
 * it into each utility and emits no `--color-*` variable of its own, so anything
 * reading a colour from JavaScript looks up `primary`, never `color-primary`.
 */
export type RawToken = string;

/** One theme's tokens — every {@link RawToken} it sets, mapped to its oklch string. */
...
/**
 * The seven shadcn base colours, transcribed byte for byte.
 *
 * A base colour is the entire page — surface, text, border, chart ramp and
 * sidebar — which is why it is the only axis carrying a full token set. The
 * oklch strings are copied out of shadcn's own `THEMES` rather than re-derived,
 * because a ramp that has drifted a hundredth of a lightness step from the one
 ...
 * them at neutral's values the moment a different ramp was applied.
 */
