...
`delacour.json` lives wherever the components live. What moves is `app.root`, and with it every
file the CLI has to touch but does not own.

<Tabs items={["In the app", "In a shared package"]}>
<Tab value="In the app">

```json title="apps/mobile/delacour.json"
{
...
```json title="packages/ui/delacour.json"
{
  "paths": { "ui": "src/components/ui", "lib": "src/lib" },
...
Point each app's Tailwind entry at the package and give each its own Metro wrapper. `delacour.json`
...
<Callout type="warn">
`--cwd apps/tablet` finds the nearest `delacour.json` walking upwards. If your second app has no
config of its own, it resolves the shared package's — which names the *first* app in `app.root`.
Give each app a `delacour.json` when they diverge.
</Callout>
