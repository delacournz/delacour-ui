...
Apart from the analytics (see [Analytics](#analytics)), this is the only third-party origin the
site loads from. The `preconnect` pair is there because
...
`scripts/screenshots.ts` holds the shot list — thirty-one frames covering the landing page, two component
pages and the button's install block, the components index, the installation pages, the customiser, the 404, `/compare/heroui`, `/privacy` and the cookie banner, in both themes, at
