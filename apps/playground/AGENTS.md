...
├── tokens.ts                     the spacing and motion the app's own chrome shares — see Tokens
...
anywhere. The store still persists it, but a fresh install opens **dark** — see
[Two defaults](#two-defaults). `GalleryScreen` carries the same action, so the
...
There are two configurations this app calls "default", and they are deliberately
not the same one:
...
`src/design-system/store.pure.ts`, so `bun test` reaches them without React
Native; `store.ts` is what is left once MMKV and Uniwind are added.
...
### The data is data, and it lives in a package now

`@delacour/design-system` holds the axes, the resolver, the preset codec and the CSS emitters —
plain objects importing nothing from React Native, which is what lets `design-system.test.ts` load
them. It asserts that every name any axis writes is one `theme.css` actually declares; a token the
library renamed, or a web-only token the transcription dragged along, is otherwise invisible: it
sits in the store forever and no component reads it.

It moved out of this app because `apps/web` needs the same answers to render `/theme`. What stays
here is `src/design-system/store.ts` — MMKV plus `Uniwind.updateCSSVariables` — and
`preset-url.ts`, which builds the link the footer opens. The package's own
[AGENTS.md](../../packages/design-system/AGENTS.md) carries the rest, including why geometry is
numbers here and units in the CSS, and why a preset code can never be renumbered.

**Geometry is numbers, never `"44px"` strings.** Uniwind's `createVarGetter` parses colours with
culori and passes everything else through unchanged, so a length reaches React Native as a string
and `height`, `borderRadius` and `fontSize` ignore it with no error. Values in the CSS escape this
...
its height into the screen context and *both* tabs' scroll areas read it, so one footer reserves its
own room in both with nothing said in either tab file. One per tab would be two writers on one
shared value, racing across a pager swipe.
...
the thumb beside the one people came for — least of all on the Preview tab, where it would act on
...
scrollers (`axis-strip.tsx` is the shared shell), and the preset strip above
them is a sixth built on the same shell. The split is not arbitrary:
...
| --- | --- | --- |
| Presets | a miniature of the app: a `card` surface at the preset's corner, `Aa` in its heading face, a `primary` pill and a `muted-foreground` bar | a preset writes all seven axes, so nothing smaller than a whole surface says what it changes. Its options are `PRESET_SHORTCUTS`, shared with the documentation site's presets row |
...
`src/tokens.ts` holds the spacing and motion the app's own chrome shares:
`LIST_GAP` (the `gap-6` between the blocks on a scroll area, and
`LIST_GAP_POINTS` for the one spacer that has to give it back in a style),
`SECTION_GAP` (a heading and the group under it), `SPECIMEN_HEIGHT` (the box
every theme strip draws its tile in) and `FADE` (the label crossfade, out
before in, opacity only). Each was a literal copied into several files —
eleven `gap-6`, four `SPECIMEN_HEIGHT = 56`, two crossfade pairs — and
`tokens.test.ts` fails by file name when one is written out again.

The demos are exempt. They are published source, and a reader copying one into
their app should not need a file their app does not have.

The library's own spacing stays in `theme.css` and reaches a component as a
class; nothing here restates a library token.
...
- **`expo-splash-screen` must always be given an `image`.** Its Android plugin
  ...
  sRGB of the house preset's `--background` — see [Two defaults](#two-defaults).
  It also fails if `generate-icons.ts`
