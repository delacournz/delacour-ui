/**
 * The commander program, built but not parsed.
 *
 * Split from `index.ts` so the command surface can be *read* as data — by
 * `commandManifest`, and through it by the test that holds every `delacour …`
 * printed in the documentation to a command this CLI actually has. Importing
 * the entry point to get at it would parse `process.argv` as a side effect.
 *
 * `index.ts` is now the shebang and the parse, and nothing else.
 *
...
withRegistryOptions(
	program
		...
		// the project up. The default is `src`, which is where `with-router-uniwind`
		// and most Expo templates keep their files; `--src .` is for the blank ones
		// that keep theirs at the project root.
